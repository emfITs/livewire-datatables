<div wire:click.prefetch="toggle('{{ $index }}')"
     ...
     >
     <button type="button" class="relative h-12 w-3 focus:outline-none">
         <span
...
<div class="@if($column['hidden']) hidden @else relative h-12 overflow-hidden align-top flex table-cell @endif" @include('datatables::style-width')>
    ...
    @if($column['sortable'])
        <button type="button" wire:click="sort('{{ $index }}')"
    ...
    @if ($column['hideable'])
        <button type="button" wire:click.prefetch="toggle('{{ $index }}')"
