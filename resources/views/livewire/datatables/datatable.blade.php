    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
                    ...
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <div class="absolute inset-y-0 right-0 flex items-center pr-2">
                                <button type="button" wire:click="$set('search', null)" class="text-gray-300 hover:text-red-600 focus:outline-none">
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if($this->activeFilters)
                    <button type="button" wire:click="clearAllFilters" class="flex items-center px-3 text-xs font-medium tracking-wider text-red-500 uppercase bg-white border border-red-400 space-x-2 rounded-md leading-4 hover:bg-red-200 focus:outline-none"><span>{{ __('Reset') }}</span>
                ...
                @if($exportable)
                    <div x-data="{ init() {
                        ...
                        <button type="button" wire:click="export" class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 space-x-2 rounded-md leading-4 hover:bg-green-200 focus:outline-none"><span>{{ __('Export') }}</span>
                ...
                @foreach ($columnGroups as $name => $group)
                    <button type="button" wire:click="toggleGroup('{{ $name }}')"
        ...
        @if($hideable === 'buttons')
            <div class="p-2 grid grid-cols-8 gap-2">
                @foreach($this->columns as $index => $column)
                    @if ($column['hideable'])
                        <button type="button" wire:click.prefetch="toggle('{{ $index }}')" class="px-3 py-2 rounded text-white text-xs focus:outline-none
